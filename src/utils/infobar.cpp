...
void Infobar::draw()
{
	if (active)
	{
		...
		glBegin(GL_QUADS);
			...
			glVertex2f(*settings->winWidth, 0.0f);
			glVertex2f(*settings->winWidth, barheight);
		glEnd();
		glDisable(GL_BLEND);

		glColor3f(1.0f, 1.0f, 1.0f);

		float col1 = (*settings->winWidth/4);
...
// 		unsigned int totalneurons = settings->info_totalNeuronsH + settings->info_totalNeuronsC;
// 		unsigned int totalsynapses = settings->info_totalSynapsesH + settings->info_totalSynapsesC;
	...
	// Row 1
		...
		Textprinter::Instance()->printR(col2-hsp,	vsp,	"%1u",		settings->info_critters);

		Textprinter::Instance()->print(col2+hsp,	vsp,	"food:");
		Textprinter::Instance()->printR(col3-hsp,	vsp,	"%1u/%1u",	settings->info_food, (unsigned int)(settings->freeEnergyInfo/settings->getCVar("food_maxenergy")));
